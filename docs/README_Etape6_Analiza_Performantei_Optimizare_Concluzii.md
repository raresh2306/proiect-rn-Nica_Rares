...
- Etapa 6 = Model OPTIMIZAT + Aplicație MATURIZATĂ + Concluzii industriale
...
|----------|------------------------------------------|--------------|--------------|-------------------|----------------|
| **Baseline** | MLP simplu (64 neuroni), fără Dropout, LR fix | 0.85 | 0.82 | 8 min | Modelul face overfitting rapid; Loss-ul de validare crește după epocile inițiale. |
| **Exp 1** | Arhitectură extinsă: [128, 64] + Dropout 0.2 | 0.92 | 0.90 | 12 min | Dropout-ul a redus overfitting-ul, iar capacitatea crescută a rețelei a îmbunătățit învățarea trăsăturilor fine. |
| **Exp 2** | Adăugare Scheduler: `ReduceLROnPlateau` | 0.96 | 0.95 | 15 min | Ajustarea dinamică a ratei de învățare a permis convergența către un minim mai bun al funcției de loss. |
| **Exp 3** | **+ Augmentare Date: Gaussian Noise (0.005)** | **1.00** | **1.00** | **18 min** | **BEST** - Introducerea zgomotului a forțat modelul să fie robust la variații, eliminând erorile reziduale. |

**Justificare alegere configurație finală:**
```text
Am ales configurația din Exp 3 ca model final pentru că:

1. **Performanță Maximă:** Oferă metrici perfecte pe setul de test (Accuracy 1.0, F1-score 1.0), conform fișierului `results/test_metrics.json`, asigurând o clasificare precisă a gesturilor.
2. **Robustețe la Zgomot:** Îmbunătățirea decisivă vine din injectarea de zgomot Gaussian (nivel 0.005) direct pe coordonatele landmarks-urilor. Aceasta simulează imperfecțiunile senzorului web și tremurul mâinii operatorului, prevenind overfitting-ul pe poziții statice.
3. **Stabilitate în Antrenare:** Utilizarea scheduler-ului `ReduceLROnPlateau` a permis o convergență fină, evitând oscilațiile loss-ului în epocile finale.
4. **Eficiență Computațională:** Deși am crescut complexitatea (128 neuroni), timpul de inferență rămâne sub 30ms, critic pentru controlul în timp real al robotului.
```

**Resurse utilizate pentru Optimizare (Documentație Oficială PyTorch):**
- **Scheduler (folosit în `train.py`):** `ReduceLROnPlateau` - pentru ajustarea dinamică a ratei de învățare.
  Link: https://pytorch.org/docs/stable/generated/torch.optim.lr_scheduler.ReduceLROnPlateau.html
- **Regularizare (folosit în `model.py`):** `torch.nn.Dropout` - pentru prevenirea overfitting-ului.
  Link: https://pytorch.org/docs/stable/generated/torch.nn.Dropout.html
- **Optimizator (folosit în `hyperparameters.yaml`):** `torch.optim.Adam` - algoritmul de gradient descent.
